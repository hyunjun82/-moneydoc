...
/**
 * 모든 계산기 일괄 검증
 * - moneydoc-data/calculators/ 안의 모든 JSON 로드
 * - 각 계산기의 산식 함수로 verification.cases 검증
 * - expected ≡ actual (±2원 토러런스) 확인
 */
const fs = require('fs');
...
function calc_salary(input, data) {
  ...
  for (const b of T.earnedIncomeDeduction.brackets) {
    if (b.upperBound === null || annual <= b.upperBound) {
      eid = b.fixed + (annual - b.lowerBound) * b.rate;
      break;
    }
  }
  ...
  const taxableIncome = max(0, annual - eid - personalDeduction);
  ...
  for (const c of T.earnedIncomeTaxCredit.computation) {
    if (c.salaryThreshold === null || taxBeforeCredit <= c.salaryThreshold) {
      creditRaw = c.fixed + (taxBeforeCredit - c.lowerSalary) * c.rate;
      break;
    }
  }
  ...
  for (const l of T.earnedIncomeTaxCredit.limits) {
    if (l.totalSalaryUpper === null || annual <= l.totalSalaryUpper) {
      if (l.type === 'fixed') creditLimit = l.amount;
      else creditLimit = max(l.minLimit, l.base - (annual - l.subtractAfter) * l.subtractRate);
      break;
    }
  }
  const earnedIncomeCredit = min(creditRaw, creditLimit);
...
// ═══════════════════════════════════════════════════════════════
function calc_dsr(input, data) {
  const { annualIncome, monthlyExistingDebt, loanYears, loanRate, dsrLimit } = input;
  const monthlyAvailable = round((annualIncome * dsrLimit) / 12 - monthlyExistingDebt);
  if (monthlyAvailable <= 0) return { monthlyAvailable: 0, maxLoan: 0 };
  const r = loanRate / 12;
  const n = loanYears * 12;
  const pow = Math.pow(1 + r, n);
  const factor = (pow - 1) / (r * pow);
  const maxLoan = round(monthlyAvailable * factor);
  return { monthlyAvailable, maxLoan };
}
...
function calc_installmentSavings(input, data) {
  const { monthlyDeposit, months, rate } = input;
...
function calc_loanAmortization(input) {
  ...
  const monthly = round(M);
  const totalPayment = round(M * n);
...
function calc_ltvLimit(input, data) {
  const { housePrice, category, adjusted } = input;
  const rule = data.tables.ltvRates.rules.find(
    r => r.category === category && r.adjusted === adjusted
  );
  const ltv = rule ? rule.ltv : 0;
  const maxLoan = round(housePrice * ltv);
  return { ltv, maxLoan };
}
...
function calc_propertyTax(input, data) {
  const C = data.constants, T = data.tables;
  const { publicValue } = input;
  const taxableBase = round(publicValue * C.FAIR_MARKET_RATIO);
  let propertyTax = 0;
...
function calc_taxFreeSavings(input) {
  ...
  return { principal: p, interest, maturity: p + interest };
}
function calc_isaTaxSaving(input, data) {
  const C = data.constants;
  const generalTax = round(input.totalInterest * C.GENERAL_RATE);
  const isaTax = input.totalInterest <= C.ISA_EXEMPT_LIMIT ? 0 : round((input.totalInterest - C.ISA_EXEMPT_LIMIT) * C.ISA_RATE);
...
function calc_dtiLimit(input) {
  const monthlyAvail = round((input.annualIncome * input.dtiLimit) / 12 - input.monthlyOtherDebtInterest);
  if (monthlyAvail <= 0) return { monthlyAvail: 0, maxLoan: 0 };
  const r = input.loanRate / 12, n = input.loanYears * 12;
  const pow = Math.pow(1 + r, n);
  const factor = (pow - 1) / (r * pow);
  return { monthlyAvail, maxLoan: round(monthlyAvail * factor) };
}
function calc_jeonseLoan(input) {
  const calc = round(input.deposit * input.ratio);
  return { maxLoan: min(calc, input.limit) };
}
function calc_mortgageLoanLimit(input) {
  const ltvLimit = round(input.housePrice * input.ltv);
  const monthlyAvail = round((input.annualIncome * input.dsrLimit) / 12 - input.monthlyExistingDebt);
  let dsrLimitAmount = 0;
  if (monthlyAvail > 0) {
    const r = input.loanRate / 12, n = input.loanYears * 12;
    const pow = Math.pow(1 + r, n);
    dsrLimitAmount = round(monthlyAvail * (pow - 1) / (r * pow));
  }
  return { ltvLimit, dsrLimitAmount, maxLoan: min(ltvLimit, dsrLimitAmount) };
}
function calc_prepaymentFee(input) {
  return { fee: round(input.remainingBalance * input.rate * (input.remainingMonths / input.totalMonths)) };
}
...
function calc_housingSubscriptionScore(input, data) {
  ...
  const accountScore = min(C.MAX_ACCOUNT, input.accountYears * 1);
...
function calc_subscriptionPriority(input, data) {
  const C = data.constants;
  const accountOk = input.accountMonths >= C.MIN_ACCOUNT_MONTHS;
  const depositOk = input.depositCount >= C.MIN_DEPOSITS;
  const noHomeOk = input.isNoHome;
  return { accountOk, depositOk, noHomeOk, isEligible: accountOk && depositOk && noHomeOk };
}
...
const calculators = {
  ...
  'youth-hope-deposit': calc_youthHopeDeposit,
  ...
  'registration-fee': calc_registrationFee,
  ...
  'health-insurance-employee': calc_healthInsuranceEmployee,
  'health-insurance-regional': calc_healthInsuranceRegional,
  ...
  'driver-insurance': calc_driverInsurance,
  'life-insurance-surrender': calc_lifeInsuranceSurrender,
  ...
  'buy-vs-jeonse': calc_buyVsJeonse,
  'wedding-cost': calc_weddingCost,
  'child-tuition-sim': calc_childTuitionSim,
  'savings-vs-fund': calc_savingsVsFund,
  'child-rearing-cost': calc_childRearingCost,
...
function loadAllCalculators() {
  ...
  const all = [];
  for (const cat of fs.readdirSync(calcDir)) {
    const catDir = path.join(calcDir, cat);
    if (!fs.statSync(catDir).isDirectory()) continue;
    for (const file of fs.readdirSync(catDir)) {
      if (file.endsWith('.json')) {
        const fp = path.join(catDir, file);
        all.push(JSON.parse(fs.readFileSync(fp, 'utf-8')));
      }
    }
  }
  return all;
...
function compareValue(exp, act, key) {
  if (exp === undefined || act === undefined) return { ok: true };
  if (typeof exp === 'boolean') {
    return { ok: exp === act, diff: exp !== act ? `expected ${exp}, actual ${act}` : '' };
  }
  if (key === 'rate' || key === 'ltbcRate') {
    const diff = Math.abs(exp - act);
    if (diff > 0.0005) return { ok: false, diff: `expected ${exp}, actual ${act} (diff ${(act - exp).toFixed(6)})` };
    return { ok: true };
  }
  if (key === 'deductRatePct') {
    return { ok: Math.abs(exp - act) <= 0.2, diff: `expected ${exp}, actual ${act}` };
  }
  const diff = Math.abs(exp - act);
  if (diff > TOL) return { ok: false, diff: `expected ${exp.toLocaleString()}, actual ${act.toLocaleString()} (diff ${(act - exp).toLocaleString()})` };
  return { ok: true };
}

function verifyOne(data) {
  const fn = calculators[data.id];
  if (!fn) {
    console.log(`⚠ [${data.id}] 산식 함수 미등록 — 스킵`);
    return { skipped: true };
  }
  console.log(`\n┌─ ${data.title} (${data.id})`);
  ...
  for (const c of data.verification.cases) {
    const actual = fn(c.input, data);
    let caseOk = true;
    const diffs = [];
    for (const k of Object.keys(c.expected)) {
      const r = compareValue(c.expected[k], actual[k], k);
      if (!r.ok) { caseOk = false; diffs.push(`     ${k}: ${r.diff}`); }
    }
    if (caseOk) {
      console.log(`│  ✓ ${c.name}`);
      pass++;
...
console.log('═══════════════════════════════════════════════════════');
console.log(' MoneyDoc 계산기 산식 일괄 검증');
console.log('═══════════════════════════════════════════════════════');

const all = loadAllCalculators();
...
console.log('═══════════════════════════════════════════════════════');
console.log(` 전체 결과: ${totalPass} 통과 / ${totalFail} 실패 / ${skipped} 스킵`);
console.log('═══════════════════════════════════════════════════════');
