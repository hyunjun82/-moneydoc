// 자동 생성: scripts/article-template/convert-v2.mjs — 직접 수정하지 말 것
// 원본: public/_preview/article-v2-unemployment-documents-guide.html
export const meta = {
  title: "실업급여 서류, 신청 준비물 목록과 발급받는 곳",
  description: "실업급여 신청에 본인이 챙길 건 신분증과 계좌예요. 나머지는 회사가 내는 서류고요. 퇴사 사유에 따라 더 필요한 자료까지 발급처와 함께 정리했어요.",
  datePublished: "2026-09-04",
  dateModified: "2026-09-09",
  url: "https://moneydoc.kr/government/unemployment-documents-guide/",
  image: "https://moneydoc.kr/og/unemployment-documents-guide.png",
  imageAlt: "실업급여 신청 서류 목록과 발급처",
};

export const scriptKey = "unemployment-documents-guide";

export const html = `<h1>실업급여 서류, 신청 준비물 목록과 발급받는 곳</h1>
<div class="v2-meta"><span>글 한결</span><span>·</span><span>2026년 9월 기준</span><span>·</span><span>6분</span><span class="v2-badge">고용보험법 신고·제출 규정과 고용24 안내 대조 · 2026-09-09</span></div>
<img class="v2-hero" src="/og/unemployment-documents-guide.png" width="1200" height="630" alt="실업급여 신청 서류 목록과 발급처">
<p class="v2-lead v2-intro">실업급여를 신청하러 갈 때 서류를 잔뜩 준비해야 하는 줄 아는 분이 많아요. 실제로는 본인이 들고 갈 건 신분증과 계좌뿐이에요. 이직확인서와 고용보험 상실 신고는 회사가 처리하고, 구직신청과 교육은 온라인으로 해요. 대신 퇴사 사유에 따라 따로 준비할 자료가 있어요. 무엇을 어디서 구하는지 정리했어요.</p>
<section class="v2-answer" aria-label="즉답">
  <div class="v2-lbl">내 상황을 눌러 확인해 보세요</div>
  <div class="v2-chips" id="qchips" data-q='[{&quot;big&quot;:&quot;신분증·계좌&quot;,&quot;unit&quot;:&quot;들고 갈 것&quot;,&quot;sub&quot;:&quot;이 두 가지면 시작할 수 있어요&quot;},{&quot;big&quot;:&quot;먼저 신청&quot;,&quot;unit&quot;:&quot;해야 할 일&quot;,&quot;sub&quot;:&quot;신고일이 기준이라 미루면 손해예요&quot;},{&quot;big&quot;:&quot;사유 자료&quot;,&quot;unit&quot;:&quot;더 챙길 것&quot;,&quot;sub&quot;:&quot;인정 사유를 보여 주는 자료가 필요해요&quot;}]'><button type="button" data-i="0" aria-pressed="true">오늘 가려고요</button><button type="button" data-i="1" aria-pressed="false">회사 서류가 안 왔어요</button><button type="button" data-i="2" aria-pressed="false">자진퇴사예요</button></div>
  <div class="v2-big" id="qnet">신분증·계좌<small>들고 갈 것</small></div>
  <div class="v2-sub" id="qsub">이 두 가지면 시작할 수 있어요</div>
  <div class="v2-split">
    <div class="v2-box"><b>회사 서류는 회사가 내요</b><span>이직확인서와 상실 신고는 본인이 들고 가는 게 아니에요</span></div>
    <div class="v2-box"><b>사유가 있으면 자료가 필요해요</b><span>자진퇴사로 인정받으려면 그 사정을 보여 줘야 해요</span></div>
  </div>
</section>

<details class="v2-toc"><summary>목차 (6개 질문)<span>열기</span></summary><ol><li><a href="#s1">실업급여 신청 서류 목록은 무엇인가요</a></li><li><a href="#s2">각 서류는 어디서 발급받나요</a></li><li><a href="#s3">이직확인서는 누가 제출하나요</a></li><li><a href="#s4">사유별로 더 내야 하는 서류가 있나요</a></li><li><a href="#s5">방문 전에 준비물을 어떻게 챙기나요</a></li><li><a href="#faq">자주 묻는 질문</a></li></ol></details>
<section class="v2-kk" aria-label="한눈에 보는 요약">
  <div class="v2-hd"><small>한눈에 보는 준비물</small><b>핵심콕콕</b></div>
  <dl>
    <div class="v2-row"><dt>본인이 들고 갈 것</dt><dd>신분증과 본인 명의 계좌</dd></div>
    <div class="v2-row"><dt>회사가 처리할 것</dt><dd>고용보험 상실 신고와 이직확인서</dd></div>
    <div class="v2-row"><dt>온라인으로 할 것</dt><dd>고용24 구직신청과 수급자격 신청자 교육</dd></div>
    <div class="v2-row"><dt>사유별 자료</dt><dd>자진퇴사로 인정받으려면 사정을 보여 주는 자료</dd></div>
    <div class="v2-row"><dt>확인하는 곳</dt><dd>고용보험 홈페이지에서 회사 서류 처리 여부</dd></div>
    <div class="v2-row"><dt>순서</dt><dd>서류가 늦어도 신청 자체를 미루지 마세요</dd></div>
  </dl>
</section>

<h2 id="s1">실업급여 신청 서류 목록은 무엇인가요<small>본인 것과 회사 것으로 나뉘어요</small></h2>
<p class="v2-lead"><span class="v2-ans">본인이 챙길 건 신분증과 본인 명의 계좌 두 가지예요.</span> 나머지는 회사가 처리하거나 온라인으로 하는 것들이에요. 고용보험 상실 신고와 이직확인서는 회사가 고용센터에 내고, 구직신청과 수급자격 신청자 교육은 고용24에서 해요.</p>
<div class="v2-tbl"><table><caption>실업급여 필요서류 목록과 처리 주체</caption><thead><tr><th>서류</th><th>누가</th><th>어디서</th><th>왜 필요한가요</th></tr></thead><tbody>
<tr class="v2-hi"><th scope="row">신분증</th><td>본인</td><td class="v2-net">주민등록증이나 운전면허증</td><td>고용센터에서 본인 확인에 써요</td></tr>
<tr class="v2-hi"><th scope="row">본인 명의 계좌</th><td>본인</td><td class="v2-net">거래 은행</td><td>급여를 받을 계좌로 등록해요</td></tr>
<tr><th scope="row">고용보험 상실 신고</th><td>회사</td><td class="v2-net">회사가 고용센터에 신고</td><td>신고가 되어야 절차가 시작돼요</td></tr>
<tr><th scope="row">이직확인서</th><td>회사</td><td class="v2-net">회사가 고용센터에 제출</td><td>사유·단위기간·평균임금이 여기 적혀요</td></tr>
<tr><th scope="row">구직신청</th><td>본인</td><td class="v2-net">고용24 온라인</td><td>실업 신고에 구직 신청이 함께 들어가요</td></tr>
<tr><th scope="row">수급자격 신청자 교육 이수</th><td>본인</td><td class="v2-net">고용24 온라인</td><td>듣지 않으면 다음 단계로 못 가요</td></tr>
</tbody></table></div>
<p class="v2-fn">실업 신고에는 구직 신청과 수급자격 인정 신청이 함께 들어가요. 이직확인서는 회사가 제출하는 서류예요.</p>
<div class="v2-note"><b>방문 전에 온라인부터 끝내세요</b> 구직신청과 교육이 끝나 있어야 출석 신청이 이어져요. 순서를 지키면 한 번에 끝나요.</div>

<h2 id="s2">각 서류는 어디서 발급받나요<small>회사, 온라인, 공공기관</small></h2>
<p class="v2-lead"><span class="v2-ans">회사에서 받는 것, 온라인에서 하는 것, 공공기관에서 떼는 것 세 갈래예요.</span> 근로계약서와 급여명세서는 회사에 요청해요. 주민등록 초본이나 가족관계증명서는 정부24에서 뗄 수 있어요. 진단서는 진료받은 병원에서 받고요.</p>
<div class="v2-tbl v2-text"><table><caption>서류별 발급처와 창구</caption><thead><tr><th>하는 일</th><th>창구</th></tr></thead><tbody>
<tr><th scope="row">구직신청과 수급자격 신청자 교육</th><td data-l="창구">고용24<a class="v2-go" href="https://www.work24.go.kr" target="_blank" rel="noopener">고용24 바로가기</a></td></tr>
<tr><th scope="row">이직확인서 처리 여부와 피보험자격 이력 확인</th><td data-l="창구">고용보험 홈페이지<a class="v2-go" href="https://www.ei.go.kr" target="_blank" rel="noopener">고용보험 홈페이지</a></td></tr>
<tr><th scope="row">주민등록 초본, 가족관계증명서</th><td data-l="창구">정부24나 주민센터<a class="v2-go" href="https://www.gov.kr" target="_blank" rel="noopener">정부24 바로가기</a></td></tr>
<tr><th scope="row">근로계약서, 급여명세서</th><td data-l="창구">회사에 요청</td></tr>
<tr><th scope="row">진단서, 의사 소견서</th><td data-l="창구">진료받은 병원</td></tr>
<tr><th scope="row">수급자격 인정 신청</th><td data-l="창구">관할 고용센터에 출석</td></tr>
</tbody></table></div>
<p class="v2-fn">수급자격 인정 신청은 원칙적으로 고용센터에 출석해야 해요. 정해진 사유가 있을 때만 정보통신망으로 신고할 수 있어요.</p>
<div class="v2-note"><b>재직 중에 받아 두면 편해요</b> 퇴사한 뒤에는 회사 시스템에 접근할 수 없어요. 급여명세서와 근무 기록은 미리 내려받아 두세요.</div>

<h2 id="s3">이직확인서는 누가 제출하나요<small>회사가 고용센터에 내요</small></h2>
<p class="v2-lead"><span class="v2-ans">회사가 작성해 고용센터에 제출해요. 본인이 들고 가는 서류가 아니에요.</span> 법은 구직급여를 받으려는 사람이 이직확인서 발급을 요청할 수 있다고 정하고, 고용센터가 사업주에게 제출을 요청하면 사업주는 제출해야 한다고 정해요.</p>
<p><span class="v2-ans">요청하면 회사는 10일 이내에 발급해야 해요.</span> 끝까지 안 주면 300만원 이하의 과태료가 부과돼요. <a class="v2-go" href="/unemployment/confirmation/">이직확인서 발급과 정정</a></p>
<p><span class="v2-ans">처리 여부는 본인이 확인할 수 있어요.</span> 고용보험 홈페이지에서 제출됐는지, 어떤 사유로 신고됐는지 볼 수 있어요. 사유가 실제와 다르면 수급자격 판단이 달라지니 신청 전에 확인하는 게 안전해요.</p>
<div class="v2-note"><b>서류가 늦어도 신청은 하세요</b> 신고일을 기준으로 대기기간과 인정일이 정해져요. 회사 서류를 기다리다 신고가 늦어지면 그만큼 못 받는 날이 생겨요.</div>

<h2 id="s4">사유별로 더 내야 하는 서류가 있나요<small>자진퇴사라면 사정을 보여 줘야 해요</small></h2>
<p class="v2-lead"><span class="v2-ans">회사가 그만두게 한 경우가 아니라면, 인정 사유를 보여 주는 자료가 필요해요.</span> 자발적으로 그만뒀더라도 정당한 사유에 해당하면 수급자격이 인정돼요. 다만 사유가 있었다는 것을 자료로 보여 주지 못하면 인정받기 어려워요. <a class="v2-go" href="/unemployment/voluntary/">사유별 증빙 서류 전체</a></p>
<div class="v2-tbl"><table><caption>퇴사 사유별로 더 챙길 자료</caption><thead><tr><th>퇴사 사유</th><th>준비할 자료</th></tr></thead><tbody>
<tr><th scope="row">임금체불로 퇴사</th><td>급여명세서, 통장 입금 내역, 체불 진정 접수증</td></tr>
<tr><th scope="row">직장 내 괴롭힘으로 퇴사</th><td>회사 신고 기록, 조사 결과, 대화 기록</td></tr>
<tr><th scope="row">통근 곤란으로 퇴사</th><td>주민등록 초본, 사업장 이전 공지, 통근 경로 자료</td></tr>
<tr><th scope="row">질병이나 부상으로 퇴사</th><td>의사 소견서나 진단서, 사업주 의견</td></tr>
<tr><th scope="row">임신·출산·육아로 퇴사</th><td>임신 확인서, 가족관계증명서, 휴직 불허 사실</td></tr>
<tr><th scope="row">가족 간호로 퇴사</th><td>진단서, 가족관계증명서, 휴가 불허 사실</td></tr>
<tr><th scope="row">계약만료로 퇴사</th><td>근로계약서에 적힌 계약 기간</td></tr>
<tr><th scope="row">권고사직으로 퇴사</th><td>권고 사실이 담긴 문자나 메일, 희망퇴직 공고</td></tr>
</tbody></table></div>
<p class="v2-fn">고용센터가 사안에 따라 다른 자료를 요구할 수 있어요. 여기 적힌 건 사유를 보여 주기 위해 흔히 쓰이는 자료예요.</p>
<div class="v2-note"><b>자료가 부족해도 신청하세요</b> 고용센터가 추가 자료를 요청하는 경우가 많아요. 보완할 기회가 있으니 신청 자체를 미루지 마세요.</div>

<h2 id="s5">방문 전에 준비물을 어떻게 챙기나요<small>고용센터에 무엇을 가져가나요</small></h2>
<p class="v2-lead"><span class="v2-ans">회사 서류 처리 여부를 먼저 보고, 온라인 절차를 끝낸 뒤 신분증과 계좌를 챙겨 나가면 돼요.</span> 이 순서가 어긋나면 헛걸음을 하게 돼요. 특히 수급자격 신청자 교육을 듣지 않으면 출석해도 신청이 이어지지 않아요.</p>
<div class="v2-tbl v2-text"><table><caption>방문 전 확인 순서</caption><thead><tr><th>순서</th><th>확인할 것</th></tr></thead><tbody>
<tr><th scope="row">1</th><td data-l="확인할 것">회사가 고용보험 상실 신고를 했는지</td></tr>
<tr><th scope="row">2</th><td data-l="확인할 것">이직확인서가 제출됐고 사유가 맞는지</td></tr>
<tr><th scope="row">3</th><td data-l="확인할 것">고용24 구직신청이 끝났는지</td></tr>
<tr><th scope="row">4</th><td data-l="확인할 것">수급자격 신청자 교육을 이수했는지</td></tr>
<tr><th scope="row">5</th><td data-l="확인할 것">신분증과 본인 명의 계좌를 챙겼는지</td></tr>
<tr><th scope="row">6</th><td data-l="확인할 것">퇴사 사유에 따라 필요한 자료가 있는지</td></tr>
</tbody></table></div>
<p class="v2-fn">고용보험 홈페이지에서 1번과 2번을 한 번에 확인할 수 있어요.</p>
<div class="v2-note"><b>방문할 고용센터를 먼저 확인하세요</b> 거주지 관할 고용센터로 가야 해요. 방문 전에 운영 시간과 예약이 필요한지 확인해 두면 기다리는 시간을 줄일 수 있어요.</div>

<h2 id="faq">자주 묻는 질문</h2>
<div class="v2-faqs">
<details class="v2-faq" open><summary><i>Q</i><span>실업급여 신청에 무슨 서류가 필요한가요?</span></summary><div><i>A</i><p>본인이 들고 갈 건 신분증과 본인 명의 계좌예요. 이직확인서와 고용보험 상실 신고는 회사가 처리해요.</p></div></details>
<details class="v2-faq"><summary><i>Q</i><span>이직확인서를 본인이 발급받아 가야 하나요?</span></summary><div><i>A</i><p>아니에요. 회사가 고용센터에 제출하는 서류예요. 처리 여부는 고용보험 홈페이지에서 확인할 수 있어요.</p></div></details>
<details class="v2-faq"><summary><i>Q</i><span>회사 서류가 늦어지면 신청을 미뤄야 하나요?</span></summary><div><i>A</i><p>아니에요. 신분증과 계좌만 있으면 갈 수 있어요. 회사 서류는 뒤따라 처리돼요.</p></div></details>
<details class="v2-faq"><summary><i>Q</i><span>자진퇴사면 무슨 자료가 더 필요한가요?</span></summary><div><i>A</i><p>인정 사유를 보여 주는 자료가 필요해요. 임금체불이면 급여명세서와 체불 진정 접수증, 통근 곤란이면 주민등록 초본 같은 자료예요.</p></div></details>
<details class="v2-faq"><summary><i>Q</i><span>주민등록 초본은 어디서 떼나요?</span></summary><div><i>A</i><p>정부24나 주민센터에서 뗄 수 있어요.</p></div></details>
<details class="v2-faq"><summary><i>Q</i><span>교육을 안 들으면 신청이 되나요?</span></summary><div><i>A</i><p>수급자격 신청자 교육을 마쳐야 다음 단계로 갈 수 있어요. 방문 전에 온라인으로 들어 두세요.</p></div></details>
</div>
<section class="v2-sum" aria-label="정리"><div class="v2-hd"><small>이 글 한 줄 정리</small><b>정리</b></div><ul>
<li>본인이 챙길 건 신분증과 본인 명의 계좌 두 가지예요.</li>
<li>이직확인서와 고용보험 상실 신고는 회사가 처리해요.</li>
<li>구직신청과 수급자격 신청자 교육은 고용24에서 미리 끝내세요.</li>
<li>자진퇴사라면 인정 사유를 보여 주는 자료를 함께 준비하세요.</li>
</ul></section>

<h2 id="src">출처</h2>
<div class="v2-src">
<b>법령</b>고용보험법 제42조(실업의 신고, 구직 신청과 수급자격 인정신청 포함, 이직확인서 발급 요청), 제43조(수급자격의 인정과 이직확인서 제출 요청), 고용보험법 시행규칙 별표 2(정당한 이직 사유).
<b>정부 도구</b>고용24의 실업급여 신청 화면과 이직확인서 작성 안내에서 항목을 대조했어요 (2026-09-09 확인).
</div>
<div class="v2-rel"><a href="/unemployment/"><b>주제 홈</b>2026년 실업급여 얼마나 받나요</a><a href="/unemployment/confirmation/"><b>다음 질문</b>실업급여 이직확인서 발급 요청과 정정 방법</a><a href="/unemployment/apply/"><b>다음 질문</b>실업급여 신청 방법 5단계, 고용24 구직신청부터 실업인정까지</a></div>
<div id="md-inter" role="dialog" aria-modal="true" aria-label="외부 사이트로 이동">
  <div class="v2-box"><div class="v2-t">공식 페이지로 이동해요</div><div class="v2-d" id="md-inter-d">새 창에서 열려요</div>
    <div class="v2-slot" id="md-ad-slot" data-ad="interstitial"></div>
    <button class="v2-btn" id="md-inter-go">바로 이동</button></div>
</div>`;

export const faqLd = {
  "@context": "https://schema.org",
  "@type": "FAQPage",
  "mainEntity": [
    {
      "@type": "Question",
      "name": "실업급여 신청에 무슨 서류가 필요한가요?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "본인이 들고 갈 건 신분증과 본인 명의 계좌예요. 이직확인서와 고용보험 상실 신고는 회사가 처리해요."
      }
    },
    {
      "@type": "Question",
      "name": "이직확인서를 본인이 발급받아 가야 하나요?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "아니에요. 회사가 고용센터에 제출하는 서류예요. 처리 여부는 고용보험 홈페이지에서 확인할 수 있어요."
      }
    },
    {
      "@type": "Question",
      "name": "회사 서류가 늦어지면 신청을 미뤄야 하나요?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "아니에요. 신분증과 계좌만 있으면 갈 수 있어요. 회사 서류는 뒤따라 처리돼요."
      }
    },
    {
      "@type": "Question",
      "name": "자진퇴사면 무슨 자료가 더 필요한가요?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "인정 사유를 보여 주는 자료가 필요해요. 임금체불이면 급여명세서와 체불 진정 접수증, 통근 곤란이면 주민등록 초본 같은 자료예요."
      }
    },
    {
      "@type": "Question",
      "name": "주민등록 초본은 어디서 떼나요?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "정부24나 주민센터에서 뗄 수 있어요."
      }
    },
    {
      "@type": "Question",
      "name": "교육을 안 들으면 신청이 되나요?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "수급자격 신청자 교육을 마쳐야 다음 단계로 갈 수 있어요. 방문 전에 온라인으로 들어 두세요."
      }
    }
  ]
};

export const landing = {
  "hero": {
    "tag": "정부지원금",
    "line1": "실업급여 신청 서류",
    "line2": "무엇을 챙기나요",
    "sub1": "본인이 챙길 건 신분증과 본인 명의 계좌예요",
    "sub2": "이직확인서와 상실 신고는 회사가 처리해요",
    "foot": "고용보험법 신고·제출 규정과 고용24 안내 대조 · 2026-09-09 검증",
    "card": {
      "label": "본인이 챙길 것",
      "big": "2",
      "unit": "가지",
      "l1": "신분증과 본인 명의 계좌",
      "l2": "나머지는 회사와 온라인으로 처리해요"
    },
    "alt": "실업급여 신청 서류 목록과 발급처"
  },
  "badge": "고용보험법 신고·제출 규정과 고용24 안내 대조 · 2026-09-09",
  "basis": "2026년 9월 기준",
  "readMinutes": 6,
  "quick": [
    {
      "chip": "오늘 가려고요",
      "big": "신분증·계좌",
      "unit": "들고 갈 것",
      "sub": "이 두 가지면 시작할 수 있어요",
      "selected": true
    },
    {
      "chip": "회사 서류가 안 왔어요",
      "big": "먼저 신청",
      "unit": "해야 할 일",
      "sub": "신고일이 기준이라 미루면 손해예요",
      "selected": false
    },
    {
      "chip": "자진퇴사예요",
      "big": "사유 자료",
      "unit": "더 챙길 것",
      "sub": "인정 사유를 보여 주는 자료가 필요해요",
      "selected": false
    }
  ],
  "boxes": [
    {
      "title": "회사 서류는 회사가 내요",
      "text": "이직확인서와 상실 신고는 본인이 들고 가는 게 아니에요"
    },
    {
      "title": "사유가 있으면 자료가 필요해요",
      "text": "자진퇴사로 인정받으려면 그 사정을 보여 줘야 해요"
    }
  ],
  "keyPoints": {
    "title": "한눈에 보는 준비물",
    "rows": [
      [
        "본인이 들고 갈 것",
        "신분증과 본인 명의 계좌"
      ],
      [
        "회사가 처리할 것",
        "고용보험 상실 신고와 이직확인서"
      ],
      [
        "온라인으로 할 것",
        "고용24 구직신청과 수급자격 신청자 교육"
      ],
      [
        "사유별 자료",
        "자진퇴사로 인정받으려면 사정을 보여 주는 자료"
      ],
      [
        "확인하는 곳",
        "고용보험 홈페이지에서 회사 서류 처리 여부"
      ],
      [
        "순서",
        "서류가 늦어도 신청 자체를 미루지 마세요"
      ]
    ]
  },
  "sections": [
    {
      "id": "s1",
      "h2": "실업급여 신청 서류 목록은 무엇인가요",
      "sub": "본인 것과 회사 것으로 나뉘어요"
    },
    {
      "id": "s2",
      "h2": "각 서류는 어디서 발급받나요",
      "sub": "회사, 온라인, 공공기관"
    },
    {
      "id": "s3",
      "h2": "이직확인서는 누가 제출하나요",
      "sub": "회사가 고용센터에 내요"
    },
    {
      "id": "s4",
      "h2": "사유별로 더 내야 하는 서류가 있나요",
      "sub": "자진퇴사라면 사정을 보여 줘야 해요"
    },
    {
      "id": "s5",
      "h2": "방문 전에 준비물을 어떻게 챙기나요",
      "sub": "고용센터에 무엇을 가져가나요"
    }
  ],
  "faq": [
    {
      "q": "실업급여 신청에 무슨 서류가 필요한가요?",
      "a": "본인이 들고 갈 건 신분증과 본인 명의 계좌예요. 이직확인서와 고용보험 상실 신고는 회사가 처리해요."
    },
    {
      "q": "이직확인서를 본인이 발급받아 가야 하나요?",
      "a": "아니에요. 회사가 고용센터에 제출하는 서류예요. 처리 여부는 고용보험 홈페이지에서 확인할 수 있어요."
    },
    {
      "q": "회사 서류가 늦어지면 신청을 미뤄야 하나요?",
      "a": "아니에요. 신분증과 계좌만 있으면 갈 수 있어요. 회사 서류는 뒤따라 처리돼요."
    },
    {
      "q": "자진퇴사면 무슨 자료가 더 필요한가요?",
      "a": "인정 사유를 보여 주는 자료가 필요해요. 임금체불이면 급여명세서와 체불 진정 접수증, 통근 곤란이면 주민등록 초본 같은 자료예요."
    },
    {
      "q": "주민등록 초본은 어디서 떼나요?",
      "a": "정부24나 주민센터에서 뗄 수 있어요."
    },
    {
      "q": "교육을 안 들으면 신청이 되나요?",
      "a": "수급자격 신청자 교육을 마쳐야 다음 단계로 갈 수 있어요. 방문 전에 온라인으로 들어 두세요."
    }
  ],
  "related": [
    {
      "kind": "주제 홈",
      "label": "2026년 실업급여 얼마나 받나요",
      "href": "/unemployment/"
    },
    {
      "kind": "다음 질문",
      "label": "실업급여 이직확인서 발급 요청과 정정 방법",
      "href": "/unemployment/confirmation/"
    },
    {
      "kind": "다음 질문",
      "label": "실업급여 신청 방법 5단계, 고용24 구직신청부터 실업인정까지",
      "href": "/unemployment/apply/"
    }
  ]
};
