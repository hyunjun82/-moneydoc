...
| `scripts/article-template/answer-check.mjs` (빌드 시점) | **회피 답**: 소제목의 답이 "사람마다 달라요" 면 근거에서 답 후보를 찾아 눈앞에 들이대고 빌드를 막는다 · **검산**: 한 문장에 일수와 총액이 같이 있으면 엔진 일액으로 곱해 본다 · **중복 문장**: 형제 글끼리 문장을 베꼈는지 |
| `scripts/prose-scan.mjs` | 한자투·명사형 종결·"여부를 확인"·"및"·해요체 이탈을 훑는다. 법령 인용과 출처는 뺀다 |
| `scripts/selftest/gate.mjs` · `hook.mjs` · `build-checks.mjs` | 장치가 진짜 잡는지 **고의 위반으로 실측**한다 (게이트 8/8 · 훅 13/13 · 빌드 4/4). gate.mjs 나 훅을 고쳤으면 반드시 돌린다. "작동한다" 는 말 대신 이 숫자를 댄다 |
...
### 10-5. 왜 세 번 일하게 됐나 (2026-09-04, 다시 그러지 않으려고 적는다)

글을 쓰고 → 나중에 문제를 찾고 → 다시 고쳤다. 같은 글을 세 번 만졌다. 원인은 하나다.

**검사가 "쓴 것" 만 보고 "안 쓴 것" 은 안 봤다.**

`factcheck` 는 글에 적힌 숫자가 근거에 있는지 봤다. 인용이 원문 그대로인지도 봤다.
그런데 *근거에 답이 있는데 글이 회피했는지* 는 아무도 묻지 않았다. 그래서 이런 게 통과했다.
...
`node scripts/selftest/build-checks.mjs` 가 이 넷을 고의로 되살려 4/4 로 막히는지 실측한다.
지금은 `answer-check.mjs` 가 **빌드 시점에** 이 넷을 다 잡는다. 글을 고치는 그 자리에서 걸린다.
게이트(커밋 시점)까지 가지 않는다. 실측으로 확인했다: 옛 오류를 되살리면 빌드가 멈춘다.

**남은 원칙 하나.** 글은 한 편씩 끝내고 다음으로 간다(10장 7번). 28편을 몰아 쓰고 나중에 검토하니
세 번 일하게 됐다. 빌드가 통과해도 `review` 를 그 자리에서 적어야 다음 글로 넘어간다.
