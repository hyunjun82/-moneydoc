...
function calc_loanAmortization(input) {
  ...
  if (mode === 'amortization') {
    const pow = Math.pow(1 + r, n);
    const M = r > 0 ? principal * r * pow / (pow - 1) : principal / n;
    const monthly = round(M);
    const totalPayment = round(M * n);
    return { mode, monthly, totalPayment, totalInterest: totalPayment - principal };
    ...
    return { mode, firstPayment, lastPayment, totalInterest, totalPayment: principal + totalInterest };
  } else if (mode === 'balloon') {
    const monthlyInterest = round(principal * r);
    const totalInterest = round(monthlyInterest * n);
    return { mode, monthlyInterest, totalInterest, totalPayment: principal + totalInterest };
  } else if (mode === 'grace') {
    const graceMonths = graceYears * 12;
    const repayMonths = n - graceMonths;
    if (repayMonths <= 0) return { mode, error: 'repayYears must be > 0' };
    ...
    const M = r > 0 ? principal * r * pow / (pow - 1) : principal / repayMonths;
    const repayMonthly = round(M);
    const totalRepayPayment = round(M * repayMonths);
    const totalInterest = totalGraceInterest + totalRepayPayment - principal;
    return { mode, graceMonthlyInterest, totalGraceInterest, repayMonthly, totalInterest, totalPayment: principal + totalInterest };
  }
  return { mode, error: 'unknown mode' };
}
