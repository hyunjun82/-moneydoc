...
function calc_salary(input, data) {
  ...
  const _ltRaw = monthlyIncomeTax * C.LOCAL_TAX_RATE;
  const _ltFlr = Math.floor(_ltRaw);
  const _ltDiff = _ltRaw - _ltFlr;
  const monthlyLocalTax = _ltDiff < 0.5 ? _ltFlr : (_ltDiff > 0.5 ? _ltFlr + 1 : (_ltFlr % 2 === 0 ? _ltFlr : _ltFlr + 1));
...
function calc_compIncome(input, data) {
  ...
  const decisionTax = round(max(0, taxBeforeCredit - childCredit - extraTaxCredit));
...
function calc_nationalPension(input, data) {
  ...
  const monthlyPension = round(C.PROPORTION_2026 * (C.A_VALUE_2026 + avgIncome) * P / 12);
...
function calc_stockTransferTax(input, data) {
  ...
  // 소득세율: 일반 20%, 대주주 + 3억 초과 = 전체 25%
  const incomeRate = (isMajor && gain > C.MAJOR_HIGH_THRESHOLD) ? 0.25 : 0.20;
...
function calc_maternityLeavePay(input) {
  ...
  const upper = round(2200000 / 30);  // 2026 출산휴가 상한
...
function calc_durunuriSupport(input, data) {
  const C = data.constants;
  const npRate = C.NP_RATE || 0.045;
  const eiRateEmp = C.EI_RATE_EMPLOYEE || 0.009;
  const eiRateEr = C.EI_RATE_EMPLOYER || 0.0115;
  const employeeNP = round(input.salary * npRate);
  const employeeEI = round(input.salary * eiRateEmp);
  const employerNP = round(input.salary * npRate);
  const employerEI = round(input.salary * eiRateEr);
