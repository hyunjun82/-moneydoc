...
function calc_salary(input, data) {
  ...
  if (monthly_salary < 410000) pension_deduction = 410000 * 0.0475 * 12;
  else if (monthly_salary > 6590000) pension_deduction = 6590000 * 0.0475 * 12;
  else pension_deduction = cutOff(cutOff(monthly_section, 1000) * 0.0475, 10) * 12;
  ...
  if (annual_salary <= 30000000) {
    ...
    if (dep === 1) special_deduction = 3600000 + Math.floor(annual_salary * 0.5 / 100);
  ...
  // 4대보험 요율: 2026년 9월 기준 (국민연금 4.75%/상한 659만·하한 41만, 건보 3.595%,
  // 장기요양 건보료의 13.14%, 고용보험 0.9%) — 국민연금공단·건강보험공단·고용노동부 고시
  ...
  const nationalPension = cutOff(Math.floor(npBaseM * 0.0475), 10);
  const healthInsurance = cutOff(Math.floor(monthly_salary * 0.03595), 10);
  const longTermCare = cutOff(Math.floor(healthInsurance * 0.1314), 10);
  const employmentInsurance = cutOff(Math.floor(monthly_salary * 0.009), 10);
  ...
  // UI 표시용 중간값 (verify-3way의 expected에는 없으므로 비교에 영향 없음)
  const eitdRaw = (calculated_tax > 500000) ? (275000 + (calculated_tax - 500000) * 0.3) : (calculated_tax * 0.55);
  ...
  return {
    ...
    earnedIncomeDeduction: Math.floor(earned_deduction),
    personalDeduction: personal_allowance,
    taxableIncome: Math.max(0, Math.floor(assessment_standard)),
    taxBeforeCredit: Math.floor(calculated_tax),
    earnedIncomeCreditRaw: Math.floor(eitdRaw),
    earnedIncomeCreditLimit: eitd_limit,
    earnedIncomeCredit: Math.floor(eitd),
    childCredit: 0,
    annualIncomeTax: Math.max(0, Math.floor(settled_tax))
  };
}
...
function calc_acqTax(input, data) {
  ...
  // 농특세 (농어촌특별세법 제5조제1항제6호): 표준세율 2%로 산출한 취득세액의 10% = 과세표준의 0.2%.
  // 국민주택규모(85㎡) 이하는 비과세(같은 법 제4조).
...
function calc_fourMajorInsurance(input, data) {
  ...
  const employeeLTC = cutOff(Math.floor(employeeHI * C.LTC_RATE), 10); // 10원 미만 절사 (연봉 계산기와 통일)
  ...
  const employerLTC = cutOff(Math.floor(employerHI * C.LTC_RATE), 10);
...
function calc_maternityLeavePay(input) {
  ...
  const cappedMonthly = min(input.monthlySalary, monthlyUpper);
  ...
  if (isSME) {
    ...
    companyPay = round(max(0, input.monthlySalary - monthlyUpper) * 60 / 30);
  } else {
    // 대기업: 60일 사업주 100% + 마지막 30/60일 정부 한도
    companyPay = round(input.monthlySalary * 60 / 30);
