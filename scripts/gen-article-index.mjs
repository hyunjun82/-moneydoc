...
import { ARTICLES } from './articles-v1-list.mjs';
...
const EXISTING = [
  { cat: 'government', catLabel: '정부지원금', href: '/gov/earned-income-tax-credit-guide/',  title: '근로장려금 지급일 2026, 8월 27일 지급',    blurb: '단독 165만 · 홑벌이 285만 · 맞벌이 330만' },
  { cat: 'government', catLabel: '정부지원금', href: '/gov/earned-income-tax-credit-check/',  title: '근로장려금 지급액 조회 방법',              blurb: '홈택스·손택스 · ARS 1544-9944' },
  { cat: 'government', catLabel: '정부지원금', href: '/gov/youth-future-savings-guide/',      title: '청년미래적금 계좌개설·가입조건·수령액',    blurb: '정부기여금 6% vs 12% 비교' },
...
const rows = [
  ...
  ...V2.map((a) => {
    ...
    const title = /title: "((?:[^"\\]|\\.)*)"/.exec(data)?.[1];
    if (!title) throw new Error(`${a.slug}: title 추출 실패`);
    return { cat: a.cat, catLabel: a.catLabel, href: hubHref(a.cat, a.slug), title: JSON.parse(`"${title}"`), blurb: a.blurb };
  }),
  ...EXISTING,
];
